...
const HolidayManage = () => {
  ...
  const fetchHolidayData = async () => {
    ...
    try {
      const { data } = await axios.get(
        `http://localhost:8080/api/getAllHolidaysCurrentYear?year=${selectedYear}`
      );
  ...
  const updateHolidayStatus = async (id, status) => {
    ...
    try {
      await axios.put(`http://localhost:8080/api/updateHolidayStatus/${id}`, { status });
  ...
  const deleteHolidays = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/api/deleteHoliday/${id}`);
