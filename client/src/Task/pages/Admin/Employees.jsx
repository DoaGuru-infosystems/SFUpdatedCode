...
const EmployeePage = ({ userRole }) => {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      await axios.put(`https://sf.doaguru.com/api/Update-Employee-Details`, updatedData);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure? This action cannot be undone.")) {
      setDeletingId(id);
      try {
        await axios.delete(`https://sf.doaguru.com/api/delete-employee/${id}`);
                  ...
                  paginatedUsers.map((user) => (
                    <tr key={user.id} className="group hover:bg-slate-50/70 transition-all transition-colors duration-150">
                      ...
                      <td className="px-3 py-2 text-center text-[10px]">
                        <span className={`px-2.5 py-1 rounded-full font-black uppercase tracking-widest text-[8px] border ${
