          ...
          {tabs.map(tab => (
            <button
              ...
              className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-black transition-all whitespace-nowrap ${
                activeTab === tab.id ? `${tab.bg} ${tab.color} shadow-sm ring-1 ring-slate-100` : 'text-slate-400 hover:text-slate-600 hover:bg-slate-50'
              }`}
            >
              {tab.icon} {tab.label}
                  ...
                  {[...Array(totalPages)].map((_, i) => (
                    <button 
                      ...
                      className={`w-8 h-8 rounded-lg text-[10px] font-black transition-all ${
                        currentPage === i+1 ? 'bg-indigo-600 text-white' : 'bg-white border border-slate-200 text-slate-500 hover:bg-slate-50'
                      }`}
                    >
                      {i+1}
                    </button>
