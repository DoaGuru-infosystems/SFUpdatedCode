                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 flex flex-col justify-between hover:shadow-md transition-shadow">
                        ...
                        <div className="mt-6 flex gap-2">
                             <label 
                                htmlFor="file-upload"
                                className={`flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold border-2 border-dashed transition cursor-pointer ${
                                    uploadFile ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-slate-200 text-slate-600 hover:border-indigo-400'
                                }`}
                            >
                                            ...
                                            <td className="py-4 px-6 text-center">
                                                <div className="flex flex-col gap-2 items-center">
                                                    <div className="flex gap-3 text-lg">
                                                        <i className={`fas fa-paper-plane transition-colors ${
                                                            (actualCounts[tgt.employeeId]?.post || 0) >= tgt.targetPost ? 'text-emerald-500 hover:text-emerald-600' : 'text-slate-200'
                                                        }`} title={`Post: ${actualCounts[tgt.employeeId]?.post || 0}/${tgt.targetPost}`}></i>
                                                        
                                                        <i className={`fas fa-video transition-colors ${
                                                            (actualCounts[tgt.employeeId]?.video || 0) >= tgt.targetVideo ? 'text-emerald-500 hover:text-emerald-600' : 'text-slate-200'
                                                        }`} title={`Video: ${actualCounts[tgt.employeeId]?.video || 0}/${tgt.targetVideo}`}></i>
                                                        
                                                        <i className={`fas fa-camera transition-colors ${
                                                            (actualCounts[tgt.employeeId]?.shoot || 0) >= tgt.targetShoot ? 'text-emerald-500 hover:text-emerald-600' : 'text-slate-200'
