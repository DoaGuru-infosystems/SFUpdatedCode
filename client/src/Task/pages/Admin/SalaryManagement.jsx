...
const SalaryManagement = ({ employeeId, baseSalary }) => {
    ...
    const fetchHistory = async () => {
        try {
            const { data } = await axios.get(`http://localhost:8080/api/salary/history/${employeeId}`);
            if (data.success) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editModeId) {
                const { data } = await axios.put(`http://localhost:8080/api/salary/update/${editModeId}`, {
                    ...formData,
                ...
                const { data } = await axios.post('http://localhost:8080/api/salary/pay', {
                    ...formData,
                    payment_duration: formattedDuration,
    ...
    const clearBalance = async (paymentId, total, currentPaid) => {
        try {
            const { data } = await axios.put(`http://localhost:8080/api/salary/update/${paymentId}`, {
                amount_paid: total,
    ...
    const deleteRecord = async (id) => {
        ...
        try {
            await axios.delete(`http://localhost:8080/api/salary/delete/${id}`);
