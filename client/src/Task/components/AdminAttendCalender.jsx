...
const AdminAttendCalendar = () => {
  ...
  const getMonthlyAttendance = async () => {
    setIsLoading(true);
    try {
      const { data } = await axios.get(`http://localhost:3000/api/getMonthlyAttendance/${month}/${year}`);
  ...
  const getHolidays = async () => {
    try {
      const { data } = await axios.get(`http://localhost:3000/api/getAllHolidaysCurrentYear`);
  ...
  const downloadReport = async () => {
    try {
      const response = await axios.get(`http://localhost:3000/api/downloadMonthlyAttendanceReport/${month}/${year}`, { responseType: "blob" });
