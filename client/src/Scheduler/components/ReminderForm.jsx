...
export default function ReminderForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    message_template: 'Hello {{employee_name}}, this is a reminder for {{title}} scheduled on {{date}} at {{time}}.',
    dnd_enabled: false,
  ...
  const filteredEmployeesSingle = (employees || []).filter(emp =>
    (emp.name || '').toLowerCase().includes(singleSearchQuery.toLowerCase()) ||
    (emp.department || '').toLowerCase().includes(singleSearchQuery.toLowerCase())
  );

  const filteredEmployeesMultiple = (employees || []).filter(emp =>
    (emp.name || '').toLowerCase().includes(multipleSearchQuery.toLowerCase()) ||
    (emp.department || '').toLowerCase().includes(multipleSearchQuery.toLowerCase())
  );

  const handleSelectAllFiltered = () => {
    setFormData(prev => {
      const currentFilteredIds = filteredEmployeesMultiple.map(emp => emp.id);
      const newIds = Array.from(new Set([...prev.employee_ids, ...currentFilteredIds]));
      return {
        ...prev,
        employee_ids: newIds
      };
  ...
  const handleClearAllFiltered = () => {
    setFormData(prev => {
      const currentFilteredIds = filteredEmployeesMultiple.map(emp => emp.id);
      const newIds = prev.employee_ids.filter(id => !currentFilteredIds.includes(id));
      return {
        ...prev,
        employee_ids: newIds
      };
      ...
      const fetchReminder = async () => {
        try {
          ...
          setFormData({
            ...
            employee_ids: data.assigned_employees ? data.assigned_employees.map(e => e.id) : [],
            title: data.title || '',
            note: data.note || '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.assignment_type === 'multiple' && formData.employee_ids.length === 0) {
    ...
    try {
      ...
      console.error(err);
      setErrorMessage(err.message || 'An error occurred while saving the reminder');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '32px' }}>
        ...
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '0.95rem', marginTop: '2px' }}>
            {isEdit ? 'Edit scheduled delivery parameters, targets, and content triggers.' : 'Define target audiences, schedule models, and messaging parameters.'}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div className="glass-card" style={{ padding: '28px', position: 'relative', zIndex: isSingleDropdownOpen ? 10 : 1 }}>
          ...
          <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
            <div className="form-group" style={{ flex: '1 1 280px', margin: 0 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <div
                    ...
                    style={{
                      ...
                      cursor: 'pointer',
                      fontSize: '0.95rem',
                      color: selectedEmployee ? 'var(--text-primary)' : 'var(--text-muted)'
                    }}
                  >
                    <span style={{ flexGrow: 1, textAlign: 'left' }}>
                      {selectedEmployee ? `${selectedEmployee.name} — ${selectedEmployee.department || 'Staff'}` : '-- Choose Team Member --'}
                    </span>
                    <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2" style={{ transform: isSingleDropdownOpen ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s', marginLeft: '8px' }}>
                          ...
                          filteredEmployeesSingle.map(emp => (
                            <div
                              ...
                              style={{
                                ...
                                fontSize: '0.9rem',
                                color: '#334155',
                                background: String(formData.employee_id) === String(emp.id) ? '#f1f5f9' : 'transparent',
                                display: 'flex',
                                justifyContent: 'space-between',
                                alignItems: 'center'
                              }}
                              onMouseEnter={(e) => e.currentTarget.style.background = '#f8fafc'}
                              onMouseLeave={(e) => e.currentTarget.style.background = String(formData.employee_id) === String(emp.id) ? '#f1f5f9' : 'transparent'}
                            >
                              <span>{emp.name}</span>
                              <span style={{ fontSize: '0.75rem', color: '#64748b', fontWeight: 600 }}>{emp.department || 'Staff'}</span>
                            </div>
                          ))
                        ) : (
                          <div style={{ padding: '12px', textAlign: 'center', fontSize: '0.875rem', color: '#94a3b8' }}>
                            No employees found
                          </div>
                        )}
                      </div>
            ...
            {formData.assignment_type === 'multiple' && (
              <div className="form-group fade-in" style={{ flex: '2 1 400px', margin: 0 }}>
                ...
                <div style={{ display: 'flex', gap: '8px', marginBottom: '8px' }}>
                  ...
                  <button
                    ...
                    className="btn btn-secondary"
                    style={{ padding: '0 12px', height: '38px', fontSize: '0.8rem', borderRadius: '8px', cursor: 'pointer' }}
                  >
                    Select All
                  </button>
                  <button
                    ...
                    className="btn btn-secondary"
                    style={{ padding: '0 12px', height: '38px', fontSize: '0.8rem', borderRadius: '8px', color: 'var(--danger)', cursor: 'pointer' }}
                  >
                    Clear All
                  </button>
                </div>
                    ...
                    filteredEmployeesMultiple.map(emp => {
                      ...
                      return (
                        <label
                          ...
                          onMouseEnter={(e) => e.currentTarget.style.background = isChecked ? 'rgba(79, 70, 229, 0.08)' : 'rgba(0, 0, 0, 0.02)'}
                        ...
                        >
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', width: '100%', alignItems: 'center' }}>
                            <span style={{ textAlign: 'left' }}>{emp.name}</span>
                            <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)', fontWeight: 600 }}>
            ...
            {formData.assignment_type === 'team' && (
              <div style={{
                ...
                <span>
                  📢 Broadcast rule: Trigger will automatically map and deliver to <strong>all active employees</strong> ({(employees || []).length} users).
                </span>
              </div>
            )}
          </div>
        ...
        <div className="glass-card" style={{ padding: '28px' }}>
          ...
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
            {/* Title */}
            <div className="form-group" style={{ flex: '2 1 340px', margin: 0 }}>
          ...
          {formData.remind_before === 'custom' && (
            <div className="form-group fade-in" style={{ marginTop: '20px', marginBottom: 0, maxWidth: '240px' }}>
          ...
          <div style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <div className="form-group" style={{ flex: '1 1 280px', margin: 0 }}>
              ...
              <select
                ...
                style={{ height: '46px' }}
              >
                <option value="whatsapp_email">WhatsApp & Email Dual Dispatch</option>
            ...
            <div style={{ flex: '2 1 400px', display: 'flex', gap: '20px', flexWrap: 'wrap', alignItems: 'center', background: '#f8fafc', padding: '16px 20px', borderRadius: '12px', border: '1px solid var(--border-glass)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                />
                <label htmlFor="dnd_enabled" style={{ cursor: 'pointer', fontSize: '0.875rem', fontWeight: 700, color: 'var(--text-secondary)' }}>
                  Silence during DND Hours
                </label>
              </div>

              {formData.dnd_enabled && (
                <div className="fade-in" style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ fontSize: '0.775rem', fontWeight: 600, color: 'var(--text-muted)' }}>From</span>
                    <input
                      type="time"
                      name="dnd_start_time"
                      ...
                      onChange={handleChange}
                      className="form-input"
                      style={{ padding: '6px 12px', width: '110px', height: '36px' }}
                    />
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <span style={{ fontSize: '0.775rem', fontWeight: 600, color: 'var(--text-muted)' }}>To</span>
                    <input
                      type="time"
                      name="dnd_end_time"
          ...
          <div className="glass-card" style={{ padding: '28px', flex: '1 1 450px' }}>
            ...
            <div className="form-group" style={{ margin: 0 }}>
              ...
              <div style={{
                ...
                <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5" style={{ color: 'var(--primary)', flexShrink: 0, marginTop: '1px' }}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                </svg>
                <span>For WhatsApp (<strong>sf_work_reminder</strong>), this payload acts as the custom message/details field (<strong>{"{{5}}"}</strong>). If left as default, your Description/Task Details will be used instead.</span>
              </div>
            </div>
          </div>
        </div>
