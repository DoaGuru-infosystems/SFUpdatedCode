...
const saveWarningLetter = async (req, res) => {
    ...
    try {
        ...
        const logoImageBytes = await loadImage(path.join(__dirname, '..', '..', 'letters', 'Controller', 'images', 'logo.png'));
        const signatureImageBytes = await loadImage(path.join(__dirname, '..', '..', 'letters', 'Controller', 'images', 'signature.png'));
    ...
    `;
        ...
        const [results] = await queryHelper(query, values);
        console.log(results);
