...
const generatePDF = (employee, res) => {
    try {
        ...
        // Add header line (green line)
        // pdfDoc.fillColor('green').rect(0, 10, pdfDoc.page.width, 5).fill(); // Header line
        // pdfDoc.fillColor('red').rect(150, 10, pdfDoc.page.width, 5).fill(); // Header line
        ...
        // Add a title
        pdfDoc.font('Helvetica-Bold').fontSize(16).text('Experience Letter', { align: 'center', underline: true });
        ...
        try {
            if (fs.existsSync(logoPath)) {
                pdfDoc.image(logoPath, 75, 15, { width: 100, height: 60 }); // Add logo at the top
            } else {
